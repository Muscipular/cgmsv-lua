...
---@param PetIndex  数值型 目标宠物对象index。
---@param ArtType  数值型 要查看的属性，具体请参考附录的宠物常量。
...
---@param PlayerIndex  数值型 目标对象index。
---@param PetIndex  数值型 目标宠物对象index。
---@return any 
...
---@param PetIndex  数值型 目标宠物对象index。
---@param SkillID  数值型 技能的ID，这里是指Tech.txt文件中定义的ID
---@return number 成功增加返回1，失败返回0。
function Pet.AddSkill(PetIndex, SkillID) end

---给宠物删除已有的指定技能。
---@param PetIndex  数值型 目标宠物对象index。
---@param SkillSlot  数值型 技能的位置，从0开始计算。
---@return number 成功增加返回1，失败返回0。
